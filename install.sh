...
readonly DOTFILES_DIR="$(realpath "$(dirname "$0")")"
...
setup_locale() {
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        sudo defaults write -g AppleLocale -string "$LOCALE" || {
            log_error "Failed to set locale $LOCALE"
            return 1
        }
    fi
...
configure_plugins() {
    ...
    if command_exists gh; then
        ...
        # Install gh extensions with error handling
        ...
        for extension in "${extensions[@]}"; do
            ...
            if ! gh extension install "$extension" --force; then
                log_warning "Failed to install gh extension: $extension"
            fi
        done
    fi
...
install_hack_nerd_font() {
    ...
    unzip -o "$temp_zip" -d "$destination" '*.ttf' || {
